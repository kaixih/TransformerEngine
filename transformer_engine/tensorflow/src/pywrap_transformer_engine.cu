...
makeTransformerEngineTensor(TFE_TensorHandle *tensor,
...
makeTransformerEngineTensor(TFE_TensorHandle *tensor) {
  transformer_engine::DType dtype =
      GetTransformerEngineDType(TFE_TensorHandleDataType(tensor));
  return makeTransformerEngineTensor(tensor, dtype);
}

transformer_engine::TensorWrapper
makeTransformerEngineTensor(const pybind11::handle tensor) {
...
TFE_TensorHandle *AllocateTensor(std::vector<int64_t> shape,
                                 TF_DataType dtype) {
  TF_Status *status = TF_NewStatus();
  ...
  TFE_TensorHandle *tensor = TFE_NewTensorHandleFromDeviceMemory(
      ctx, device_name, dtype, shape.data(), shape.size(), data, num_bytes,
...
PYBIND11_MODULE(_pywrap_transformer_engine, m) {
  ...
  m.def("cast_to_fp8",
        [](const pybind11::handle &input, const pybind11::handle &scale,
          ...
          std::vector<int64_t> shape_vec(in_shape.data,
                                         in_shape.data + in_shape.ndim);
          TFE_TensorHandle *output_eager = AllocateTensor(shape_vec, TF_INT8);
          auto output_tensor =
              makeTransformerEngineTensor(output_eager, output_dtype);
          ...
          nvte_fp8_quantize(input_tensor.data(), scale_tensor.data(),
                            output_tensor.data(), amax_out_tensor.data(),
                            scale_inv_tensor.data(), stream);
  ...
  m.def("fp8_cast_transpose_fused",
        [](const pybind11::handle &input, const pybind11::handle &scale,
          ...
          std::vector<int64_t> shape_vec(in_shape.data,
                                         in_shape.data + in_shape.ndim);
          TFE_TensorHandle *output_cast_eager =
              AllocateTensor({shape_vec[0], shape_vec[1]}, TF_INT8);
          auto output_cast_tensor =
              makeTransformerEngineTensor(output_cast_eager, output_dtype);
          TFE_TensorHandle *output_transpose_eager =
              AllocateTensor({shape_vec[1], shape_vec[0]}, TF_INT8);
          auto output_transpose_tensor =
              makeTransformerEngineTensor(output_transpose_eager, output_dtype);
          TFE_TensorHandle *scale_inv_eager = AllocateTensor({1}, TF_FLOAT);
          auto scale_inv_tensor = makeTransformerEngineTensor(scale_inv_eager);
          TFE_TensorHandle *amax_out_eager = AllocateTensor({1}, TF_FLOAT);
          auto amax_out_tensor = makeTransformerEngineTensor(amax_out_eager);
          ...
          cudaStream_t stream = 0;
          nvte_cast_transpose(
              input_tensor.data(), scale_tensor.data(),
              output_cast_tensor.data(), output_transpose_tensor.data(),
              amax_out_tensor.data(), scale_inv_tensor.data(), stream);

          PyObject *result(PyList_New(4));
          PyList_SET_ITEM(result, 0, EagerTensorFromHandle(output_cast_eager));
          PyList_SET_ITEM(result, 1,
                          EagerTensorFromHandle(output_transpose_eager));
  ...
  m.def("fp8_cast_transpose_bgrad_fused",
        [](const pybind11::handle &grad_output, const pybind11::handle &scale,
           const pybind11::handle &amax,
           const transformer_engine::DType output_dtype) {
          // Get NVTE tensors.
          ...
          std::vector<int64_t> shape_vec(in_shape.data,
                                         in_shape.data + in_shape.ndim);
          TFE_TensorHandle *grad_bias_eager = AllocateTensor(
              {shape_vec[1]}, GetTensorFlowDType(grad_output_tensor.dtype()));
          auto grad_bias = makeTransformerEngineTensor(
              grad_bias_eager, grad_output_tensor.dtype());
          TFE_TensorHandle *grad_output_cast_eager =
              AllocateTensor({shape_vec[0], shape_vec[1]}, TF_INT8);
          auto grad_output_cast =
              makeTransformerEngineTensor(grad_output_cast_eager, output_dtype);
          TFE_TensorHandle *grad_output_transpose_eager =
              AllocateTensor({shape_vec[1], shape_vec[0]}, TF_INT8);
          auto grad_output_transpose = makeTransformerEngineTensor(
              grad_output_transpose_eager, output_dtype);
          TFE_TensorHandle *scale_inv_eager = AllocateTensor({1}, TF_FLOAT);
          auto scale_inv_tensor = makeTransformerEngineTensor(scale_inv_eager);
          TFE_TensorHandle *amax_out_eager = AllocateTensor({1}, TF_FLOAT);
          auto amax_out_tensor = makeTransformerEngineTensor(amax_out_eager);
          ...
          nvte_cast_transpose_dbias(
              grad_output_tensor.data(), scale_tensor.data(),
              grad_output_cast.data(), grad_output_transpose.data(),
              amax_out_tensor.data(), grad_bias.data(), scale_inv_tensor.data(),
              workspace.data(), stream);

          // Allocate workspace
          auto workspace_shape = workspace.shape();
          std::vector<int64_t> workspace_shape_vec(workspace_shape.data,
                                                   workspace_shape.data +
                                                       workspace_shape.ndim);
          TFE_TensorHandle *workspace_data = AllocateTensor(
              workspace_shape_vec, GetTensorFlowDType(workspace.dtype()));
          ...
          PyList_SET_ITEM(result, 1,
                          EagerTensorFromHandle(grad_output_cast_eager));
          PyList_SET_ITEM(result, 2,
                          EagerTensorFromHandle(grad_output_transpose_eager));
  ...
  m.def("fp8_gemm", [](const pybind11::handle &a_handle,
                       ...
                       const bool use_bias, const pybind11::handle &bias_handle,
    ...
    transformer_engine::TensorWrapper bias(
    ...
    transformer_engine::TensorWrapper pre_gelu_out(
        nullptr, {static_cast<size_t>(1)},
        transformer_engine::DType::kBFloat16);
    ...
    TFE_TensorHandle *d_eager =
        AllocateTensor({static_cast<int64_t>(b_shape.data[0]),
                        static_cast<int64_t>(a_shape.data[0])},
                       TF_FLOAT);
    auto d = makeTransformerEngineTensor(d_eager,
                                         transformer_engine::DType::kFloat32);

    cudaStream_t stream = 0;
    nvte_cublas_gemm(a.data(), a_scale_inv.data(), b.data(), b_scale_inv.data(),
                     d.data(), bias.data(), pre_gelu_out.data(), transa, transb,
                     grad, workspace.data(), accumulate, use_split_accumulate,
                     stream);
